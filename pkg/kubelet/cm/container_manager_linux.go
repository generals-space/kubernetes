...
// caller: setupNode(), helper_linux.go -> GetRuntimeContainer()
...
func getContainer(pid int) (string, error) {
	...
	// users could enable CPU and memory accounting globally via /etc/systemd/system.conf
	// (DefaultCPUAccounting=true DefaultMemoryAccounting=true).
	// users could also enable CPU and memory accounting per unit via
	// CPUAccounting=true and MemoryAccounting=true
	// we only warn if accounting is not enabled for CPU or memory so as to
	// not break local development flows where kubelet is launched in a terminal.
	// for example, the cgroup for the user session will be something like
	// /user.slice/user-X.slice/session-X.scope, but the cpu and memory cgroup will be
	// the closest ancestor where accounting is performed (most likely /) on
	// systems that launch docker containers.
	// as a result, on those systems, you will not get cpu or memory accounting statistics for kubelet.
	// in addition, you would not get memory or cpu accounting for
	// the runtime unless accounting was enabled on its unit (or globally).
	systemd, found := cgs["name=systemd"]
	if found {
		if systemd != cpu {
