...
// Install 遍历成员对象 group 中的 Storage, 重构成有序列表后依次注册到指定前缀下.
// 返回的 WebService 对象, 有点像其他http框架中的 RouteGroup, 是一组 Route 对象的组合.
// Install handlers for API resources.
// caller: staging/src/k8s.io/apiserver/pkg/endpoints/groupversion.go -> InstallREST()
...
//////////////////////// registerResourceHandlers() 拆分到单独文件
...
// splitSubresource 检查目标路径是否包含 subresource 成员.
// 路径可以是 pod, node, 也可以是 pod/log, pod/exec 等, 后面这种是就包含 suresource 的.
// 如果没有, 那么 subresource 的返回值将为空字符串.
// splitSubresource checks if the given storage path is the path of a subresource and
// returns the resource and subresource components.
func splitSubresource(path string) (string, string, error) {
	...
	default:
		...
		return "", "", fmt.Errorf(
			"api_installer allows only one or two segment paths (resource or resource/subresource)",
		)
	}
